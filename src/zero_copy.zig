...
pub const PacketBufferPool = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, pool_size: u32, buffer_size: usize, cipher_suite: quic_crypto.QuicCrypto.CipherSuite) !PacketBufferPool {
        ...
        // Initialize each buffer with aligned memory
        for (buffers) |*buffer| {
            const aligned_memory = try allocator.alignedAlloc(u8, @as(std.mem.Alignment, @enumFromInt(12)), buffer_size); // 4KB aligned for DMA
            buffer.* = CryptoPacketBuffer.init(aligned_memory, cipher_suite);
        }
