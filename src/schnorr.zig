...
pub const MuSig2 = struct {
    ...
    pub fn nonceGen(session_id: [32]u8, private_key: ?[SCHNORR_PRIVATE_KEY_SIZE]u8, message: []const u8, extra_input: ?[]const u8) Nonce {
        var hasher = crypto.hash.sha2.Sha256.init(.{});
    ...
    pub fn partialSign(message: []const u8, private_key: [SCHNORR_PRIVATE_KEY_SIZE]u8, nonce: Nonce, agg_nonce: [66]u8, key_agg_ctx: KeyAggContext) [32]u8 {
        // Compute challenge
        var e: [32]u8 = undefined;
        var hasher = crypto.hash.sha2.Sha256.init(.{});
        hasher.update("BIP0340/challenge");
    ...
    pub fn partialSigAggregate(partial_sigs: []const [32]u8, agg_nonce: [66]u8) [SCHNORR_SIGNATURE_SIZE]u8 {
        var signature: [SCHNORR_SIGNATURE_SIZE]u8 = undefined;
...
pub const AdaptorSignature = struct {
    ...
    pub fn create(message: []const u8, private_key: [SCHNORR_PRIVATE_KEY_SIZE]u8, adaptor_point: [32]u8) !AdaptorSignature {
        // Generate nonce
    ...
    pub fn verify(self: AdaptorSignature, message: []const u8, public_key: [SCHNORR_PUBLIC_KEY_SIZE]u8, adaptor_point: [32]u8) bool {
        // R' = R + T
        var r_adapted: [32]u8 = undefined;
        var hasher = crypto.hash.sha2.Sha256.init(.{});
