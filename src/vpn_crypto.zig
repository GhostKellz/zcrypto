...
pub const VpnTunnel = struct {
    ...
    pub fn init(config: TunnelConfig) VpnTunnel {
        return VpnTunnel{
            ...
            .send_counter = 0,
            .recv_counter = 0,
            .last_key_rotation = std.time.timestamp(),
    ...
    pub fn establishTunnel(self: *VpnTunnel, local_private: [32]u8, peer_public: [32]u8) !void {
        ...
        self.send_counter = 0;
        self.recv_counter = 0;
        self.last_key_rotation = std.time.timestamp();
    }
        ...
        switch (self.config.encryption_algorithm) {
            ...
            .XChaCha20Poly1305 => {
                ...
                crypto.aead.chacha_poly.ChaCha20Poly1305.encrypt(
                    ...
                    plaintext,
                    "",
                    nonce, // Use regular nonce instead of extended
                    self.send_key
                );
                @memcpy(output[plaintext.len..plaintext.len + 16], &tag);
            },
        ...
        switch (self.config.encryption_algorithm) {
            .ChaCha20Poly1305 => {
                crypto.aead.chacha_poly.ChaCha20Poly1305.decrypt(
                    ...
                    work_slice[0..plaintext_len],
                    work_slice[plaintext_len..plaintext_len + 16][0..16].*,
                    "",
                    nonce,
                    self.recv_key
                ) catch return VpnCryptoError.HeaderProtectionFailed;
            },
            .XChaCha20Poly1305 => {
